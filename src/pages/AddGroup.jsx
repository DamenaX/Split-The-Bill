                            ...
                            {groups.map(g => (
                                <li key={g.id} className="border p-3 rounded flex justify-between items-center" onClick={() => navigate(`/groups/${g.id}`)}>
                                    ...
                                    <div className="space-x-2">
                                        <button onClick={(e) => { e.stopPropagation(); const ok = window.confirm(`Delete ${g.name}?`); if (!ok) return; deleteGroup(g.id); }} className="text-red-600">Delete</button>
                                        <Link to={`/groups/${g.id}/add-members`} onClick={(e) => e.stopPropagation()} className="text-blue-500">Add members</Link>
                                    </div>
                                </li>
                            ))}
                        </ul>
